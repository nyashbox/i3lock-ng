...
static bool load_compose_table(const char *locale) {
  ...
  if ((xkb_compose_table =
           xkb_compose_table_new_from_locale(xkb_context, locale, 0)) == NULL) {
...
static void handle_key_press(xcb_key_press_event_t *event) {
  ...
  if (xkb_compose_state && xkb_compose_state_feed(xkb_compose_state, ksym) ==
    ...
    case XKB_COMPOSE_COMPOSED:
      /* xkb_compose_state_get_utf8 doesn't include the terminating byte in the
       * return value as xkb_keysym_to_utf8 does. Adding one makes the variable
       * n consistent. */
...
static ssize_t read_raw_image_fmt(uint32_t *dest, FILE *src, size_t width,
  ...
  for (size_t y = 0; y < height; y++) {
    ...
    for (size_t x = 0; x < width; ++x) {
      int idx = x * fmt.bpp;
      dest[y * pixstride + x] = 0 | (buf[idx + fmt.red]) << 16 |
                                (buf[idx + fmt.green]) << 8 |
                                (buf[idx + fmt.blue]);
    }
  }
...
static cairo_surface_t *read_raw_image(const char *image_path,
                                       const char *image_raw_format) {
  cairo_surface_t *img;
...
#undef STRINGIFY
  ...
  if (count < size) {
    if (count < 0 || ferror(f)) {
       ...
       * but don't abort. It's useful to see how the image looks even if it's
       * wrong. */
      fprintf(stderr,
              "Warning: expected to read %zi bytes from \"%s\", read %zi\n",
...
static bool verify_png_image(const char *image_path) {
  ...
  static unsigned char PNG_REFERENCE_HEADER[8] = {137, 80, 78, 71,
                                                  13,  10, 26, 10};
  if (memcmp(PNG_REFERENCE_HEADER, png_header, sizeof(png_header)) != 0) {
    fprintf(stderr,
            "File \"%s\" does not start with a PNG header. i3lock currently "
            "only supports loading PNG files.\n",
            image_path);
    return false;
  }
  return true;
...
int main(int argc, char *argv[]) {
  ...
  if (getenv("WAYLAND_DISPLAY") != NULL) {
    errx(EXIT_FAILURE,
         "i3lock is a program for X11 and does not work on Wayland. Try "
         "https://github.com/swaywm/swaylock instead");
  }
 ...
 * is not necessarily an issue. */
  ...
  if (xkb_x11_setup_xkb_extension(conn, XKB_X11_MIN_MAJOR_XKB_VERSION,
                                  ...
                                  &xkb_base_event, &xkb_base_error) != 1) {
