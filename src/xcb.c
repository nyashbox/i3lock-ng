...
xcb_visualtype_t *get_root_visual_type(xcb_screen_t *screen) {
  ...
  for (depth_iter = xcb_screen_allowed_depths_iterator(screen); depth_iter.rem;
...
xcb_pixmap_t create_bg_pixmap(xcb_connection_t *conn, xcb_screen_t *scr,
  ...
  xcb_create_pixmap(conn, scr->root_depth, bg_pixmap, scr->root, resolution[0],
                    resolution[1]);
...
xcb_cursor_t create_cursor(xcb_connection_t *conn, xcb_screen_t *screen,
  ...
  case CURS_DEFAULT:
  default:
    return XCB_NONE; /* XCB_NONE is xcb's way of saying "don't change the
                        cursor" */
  }
  ...
  xcb_create_cursor(conn, cursor, bitmap, mask, 65535, 65535, 65535, 0, 0, 0, 0,
                    0);
...
xcb_window_t find_focused_window(xcb_connection_t *conn,
  ...
  xcb_get_property_reply_t *prop_reply = xcb_get_property_reply(
      conn,
      xcb_get_property_unchecked(conn, false, root, _NET_ACTIVE_WINDOW,
                                 XCB_GET_PROPERTY_TYPE_ANY, 0, 1 /* word */),
      NULL);
  if (prop_reply == NULL) {
    goto out;
  }
